        ...
        triggerTabList.forEach((triggerEl) => {
            ...
            if (param == "daily") {
                ...
                triggerEl.addEventListener('click', async (event) => {
                    if (idCurrent.value != undefined) idPrevious.value = idCurrent.value
                    ...
                    if (idPrevious.value == undefined) {
                        firstTimeClick = true
                        idPrevious.value = idCurrent.value
...
export async function useGetPeriods() {
    return new Promise((resolve, reject) => {
        let temp = [{
            ...
            value: "thisWeek",
            label: "This Week",
            start: Number(dayjs().tz(timeZoneTrade.value).startOf('week').add(1, 'day').unix()),
...
export function useEditItem(param) {
    ...
    if (pageId.value == "screenshots") {
        ...
        sessionStorage.setItem('screenshotIdToEdit', param)
...
export function useDateNumberFormat(param) {
    return Number(Math.trunc(param))
}
...
export function useDatetimeLocalFormat(param) {
    return dayjs.tz(param * 1000, timeZoneTrade.value).format("YYYY-MM-DDTHH:mm:ss")
}
